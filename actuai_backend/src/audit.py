...
def record(
    ...
    last = session.exec(select(AuditLog).order_by(AuditLog.id.desc()).limit(1)).first() # type: ignore
...
def verify_chain(session: Session) -> bool:
    ...
    """
    prev_hash = "GENESIS"
    for entry in session.exec(select(AuditLog).order_by(AuditLog.id.asc())): # type: ignore
