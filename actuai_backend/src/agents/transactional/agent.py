...
from etl.aog_scanner import create_aog_task
...
def _check_aog_risk(session: Session, po: PurchaseOrder, new_expected_date: date) -> None:
    ...
    if schedule is None:
        return  # no schedule on file for this part
    # Shared with the proactive ETL scan (etl/aog_scanner.py): same alert
    # shape, same dedup, different detected_by tag.
    create_aog_task(session, po, schedule, new_expected_date, detected_by="delay_email")
